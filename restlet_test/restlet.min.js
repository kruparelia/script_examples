(function e(t, n, r) {
    ...
    2: [function(require, module, exports) {
        ...
        function createQueryParameters(request) {
            if (request.queryParameters != null) {
                return _.map(request.queryParameters, function(value, name) {
                    return querystring.escape(name) + "=" + querystring.escape(value)
                }).join("&")
            } else {
                return null
            }
        ...
        function createRequestOptions(request) {
            ...
            if (request.clientInfo != null) {
                if (!_.isEmpty(request.clientInfo.acceptedMediaTypes)) {
                    debugConneg("Accepted media types: " + JSON.stringify(request.clientInfo.acceptedMediaTypes));
            ...
            if (request.entity != null) {
                if (request.entity.mediaType != null && !_.isEmpty(request.entity.mediaType.name)) {
                    options.headers["content-type"] = request.entity.mediaType.name;
                    debugResource("Sent media type: " + request.entity.mediaType.name)
                }
            }
            return options
        }
        ...
        function createHttpClient(request, handler) {
            ...
            debugHttp("Sending HTTP request with options: [ hostname=" + options.hostname + ",port=" + options.port + ",path=" + options.path + ",method=" + options.method + "]");
        ...
        transport.createClient = function(request, handler) {
            if (request.resourceRef.scheme == "http") {
                ...
                throw new Error("The protocol " + request.resourceRef.scheme + " isn't supported.")
            }
        }
    }, {
        "./client-utils": 3,
    ...
    3: [function(require, module, exports) {
        ...
        clientUtils.createResponse = function(request, rawResponse) {
            var status = new data.Status(rawResponse.statusCode, rawResponse.statusMessage);
            ...
            if (contentTypeHeader != null) {
                debugConneg("Received media type " + contentTypeHeader);
        ...
        clientUtils.setNotSupportedMediaTypeInResponse = function(response) {
            ...
            response.entity = clientUtils.createRepresentation(response.status.description, "text/plan")
        }
    }, {
        "./data": 6,
        "./headers": 7,
    ...
    4: [function(require, module, exports) {
        (function(process, global, Buffer) {
            ...
            function isBinaryMediaType(mediaType) {
                if (mediaType != null) {
                    return !(/text\/[a-z*]*/g.test(mediaType) || /application\/(xml)*/g.test(mediaType) || /application\/(json)*/g.test(mediaType) || /application\/(javascript)*/g.test(mediaType) || /application\/(atom)*/g.test(mediaType) || /application\/(yml)*/g.test(mediaType) || /application\/(smile)*/g.test(mediaType))
                }
                return true
            }
            ...
            function handleResponse(req, res, configuration, handler) {
                if (hasContent(res)) {
                    ...
                    res.entity.on("end", function() {
                        ...
                        var parameters = createParameters(configuration, req, res);
            ...
            function convertInputEntity(entity, request, response) {
                ...
                if (response.entity.mediaType == null || _.isEmpty(response.entity.mediaType.name)) {
                    clientUtils.setNotSupportedMediaTypeInResponse(response);
                    return
                }
                debugClientResource("Received media type " + response.entity.mediaType.name);
                var converter = converterApi.findConverter(response.entity.mediaType);
                if (converter == null) {
                    clientUtils.setNotSupportedMediaTypeInResponse(response);
                    return
                }
            ...
            function convertOutputEntity(request, obj, mediaType) {
                ...
                if (mediaType == null || _.isEmpty(mediaType)) {
                    clientUtils.setNotSupportedMediaTypeInResponse(response);
                    return
                }
                debugClientResource("Configured media type " + mediaType);
                var converter = converterApi.findConverter(new data.MediaType(mediaType));
                if (converter == null) {
                    clientUtils.setNotSupportedMediaTypeInResponse(response);
                    return
                }
            ...
            client.createClientResource = function(url) {
                return {
                    handle: function() {
                        var parameters = extractArgumentsForClientResourceReadWriteHandle.apply(null, arguments);
                        ...
                        if (parameters.entity != null) {
                            if (parameters.configuration.convertOutputEntity) {
                                request.entity = {};
                                convertOutputEntity(request, parameters.entity, parameters.configuration.mediaType)
                            } else {
                    ...
                    get: function() {
                        var parameters = extractArgumentsForClientResourceReadHandle.apply(null, arguments);
    ...
    6: [function(require, module, exports) {
        ...
        data.EXTENSIONS = {
            ...
            checkExtension: function(extension, mediaTypes) {
                ...
                if (configuredMediaTypes == null) {
                    ...
                    var configuredMediaType = _.find(configuredMediaTypes, function(configuredMediaType) {
                    ...
                    if (match) {
                        debugMediaType("Media type " + configuredMediaType + " match for extension " + extension)
                    } else {
                        debugMediaType("No media type match for extension " + extension)
                    }
                    return match
                }
            }
        ...
        data.Metadata.prototype = {
            isCompatible: function(otherMetadata) {
                return otherMetadata != null && (this.includes(otherMetadata) || otherMetadata.includes(this))
            }
        };
        ...
        data.MediaType.prototype = {
            ...
            getParent: function() {
                ...
                if (this.getParameters().size() > 0) {
                    result = data.MediaType.valueOf(this.getMainType() + "/" + this.getSubType())
                } else {
                        ...
                        if (this.getSubType().equals(includedMediaType.getSubType())) {
                            if (ignoreParameters) {
                                ...
                                for (var i = 0; result && i < this.parameters.size(); i++) {
                                    ...
                                    result = includedParam != null && param.value === includedParam.value
                                }
                            }
                        } else if (this.getSubType().equals("*")) {
                            result = true
                        } else if (this.getSubType().startsWith("*+") && _.endsWith(includedMediaType.getSubType(), this.getSubType().substring(2))) {
                            result = true
                        }
                    }
                }
        ...
        data.Status.prototype = {
            ...
            isConnectorError: function() {
                return this.code >= 1e3 && this.code <= 1099
            },
            isError: function() {
                return this.isClientError() || this.isServerError() || this.isConnectorError()
            },
    ...
    7: [function(require, module, exports) {
        ...
        function readClientInfo(rawRequest) {
            ...
            if (acceptCharsetHeader != null) {
                clientInfo.acceptedCharacterSets = headers.parser.readPreferenceHeaderValues(acceptCharsetHeader)
            }
            ...
            if (acceptEncodingHeader != null) {
                clientInfo.acceptedEncodings = headers.parser.readPreferenceHeaderValues(acceptEncodingHeader)
            }
            if (_.isEmpty(clientInfo.acceptedEncodings)) {
                clientInfo.acceptedEncodings.push({
                    name: "identity"
                })
            }
            var acceptLanguageHeader = rawRequest.headers["accept-language"];
            if (acceptLanguageHeader != null) {
                clientInfo.acceptedLanguages = headers.parser.readPreferenceHeaderValues(acceptLanguageHeader)
            }
            if (_.isEmpty(clientInfo.acceptedLanguages)) {
                clientInfo.acceptedLanguages.push({
                    name: "*/*"
                })
            }
            var acceptPatchHeader = rawRequest.headers["accept-patch"];
            if (acceptPatchHeader != null) {
                clientInfo.acceptedPatches = headers.parser.readPreferenceHeaderValues(acceptPatchHeader)
            }
            var expectHeader = rawRequest.headers.expect;
            if (expectHeader != null) {
                clientInfo.expectations = headers.parser.readPreferenceHeaderValues(expectHeader)
            }
